...
function jet_field($args, $ops = null, $type = 'select', $transport = 'postMessage') {
	...
	$options = [];
	if (in_array($type, ['select', 'radio', 'image']) && $ops) {
	...
	if ($type === 'color') {
		$wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, $option, [
			'label'    => __($label, 'jet'),
			'section'  => $section,
			'settings' => $option,
		]));
...
function noteControl($section, $label, $description, $id = null)
{
    ...
    $wp_customize->add_setting($id, ['sanitize_callback' => '__return_null']);
    $wp_customize->add_control(new JetToggleControl($wp_customize, $id, [
        'section' => $section,
        'label' => $label,
